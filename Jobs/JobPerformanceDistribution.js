...
const { Databases, AltScore, InspectorScoreStat } = require("../db");
...
async function UpdatePerformanceDistribution() {
    ...
    const highest_pp = await AltScore.max('pp', {
        where: {
            pp: {
                [Op.gt]: 0,
                [Op.not]: null,
    ...
    console.log(`[PP DISTRIBUTION] Got highest PP value: ${highest_pp}.`);

    //create a table of 100s from 0 to highest_pp
    const pp_table = [];
    for (let i = 0; i <= highest_pp; i += 100) {
        pp_table.push(i);
    }
